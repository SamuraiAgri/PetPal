...
struct VaccinationEntryView: View {
    ...
    @State private var showingExpiryDatePicker = false
    ...
    var body: some View {
        NavigationView {
                ...
                Section(header: Text("ワクチン情報")) {
                    ...
                    HStack {
                        ...
                        if let reminderDate = reminderDate {
                            Text(reminderDate.formattedDate)
                                .foregroundColor(.secondary)
            ...
            .sheet(isPresented: $showingExpiryDatePicker) {
                NavigationView {
                    VStack {
                        DatePicker("有効期限", selection: Binding(
                            get: { self.expiryDate ?? Calendar.current.date(byAdding: .year, value: 1, to: date)! },
                            set: { self.expiryDate = $0 }
                        ), displayedComponents: .date)
                        .datePickerStyle(GraphicalDatePickerStyle())
                        .padding()
                    }
                    .navigationTitle("有効期限を選択")
                    .navigationBarTitleDisplayMode(.inline)
                    .toolbar {
                        ToolbarItem(placement: .navigationBarTrailing) {
                            Button("完了") {
                                showingExpiryDatePicker = false
                            }
                        }
                    }
                }
            ...
            .sheet(isPresented: $showingReminderDatePicker) {
                NavigationView {
                    VStack {
                        DatePicker("リマインド日", selection: Binding(
                            get: { self.reminderDate ?? Calendar.current.date(byAdding: .month, value: 11, to: date)! },
                            set: { self.reminderDate = $0 }
                        ), displayedComponents: .date)
                        .datePickerStyle(GraphicalDatePickerStyle())
                        .padding()
                    }
                    .navigationTitle("リマインド日を選択")
                    .navigationBarTitleDisplayMode(.inline)
                    .toolbar {
                        ToolbarItem(placement: .navigationBarTrailing) {
                            Button("完了") {
